...
# CHẠY KHỞI TẠO DB TẠI ĐÂY (Đã sửa để không báo lỗi NameError)
init_db()
...
# ==========================================
# KHỞI TẠO STATE ĐĂNG NHẬP (Chỉ lưu tạm trên RAM)
# ==========================================
...
if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
                ...
                with st.form("add_m_form"):
                    ...
                    if st.form_submit_button("💾 Lưu Mới", use_container_width=True):
                        if not m_id or not m_name or not m_line: show_popup_message("LỖI", "Vui lòng nhập đủ thông tin có dấu *", "❌")
                            ...
                            conn = get_db_connection()
                            conn.execute("INSERT INTO machines VALUES (?,?,?,?,?,?)", (m_id, m_name, m_line, m_url, m_file.name if m_file else "", 1 if m_file else 0))
                            conn.commit()
                            conn.close()
        ...
        with t_edit:
            if "Chỉnh sửa" in u_perms and machine_db:
                ...
                with st.form("edit_m_form"):
                    ...
                    e_file = st.file_uploader("Thay File mẫu", disabled="File mẫu dữ liệu" not in u_edits)
                    ...
                    if st.form_submit_button("💾 Cập Nhật", use_container_width=True):
                        ...
                        conn.execute("UPDATE machines SET name=?, line=?, url=?, template_file=?, has_file=? WHERE id=?", 
                                     (e_name if "Tên máy" in u_edits else cur_m["name"],
                                      ...
                                      e_file.name if e_file else cur_m["template_file"],
                                      1 if e_file or cur_m["has_file"] else 0, sel_m))
                        conn.commit()
                        conn.close()
    ...
    # ---------------------------------------------------------
    # TRANG 3: QUẢN LÝ TÀI KHOẢN (SQLITE)
    # ---------------------------------------------------------
            ...
            with st.form("edit_user_form"):
                ...
                if st.form_submit_button("💾 Lưu Thay Đổi", use_container_width=True):
                    ...
                    conn.execute("""UPDATE users SET password_hash=?, name=?, department=?, position=?, role=?, allowed_pages=?, machine_perms=?, editable_machine_fields=? WHERE username=?""", 
                                 (hash_password(e_pass) if e_pass else cur_u["password_hash"], e_name, e_dept, e_pos, e_role, json.dumps(e_pages), json.dumps(e_m_perms), json.dumps(e_edits), sel_u))
                    conn.commit()
                    conn.close()
                    
                    if sel_u == st.session_state["username"]:
                        st.session_state["user_info"].update({"name": e_name, "department": e_dept, "position": e_pos, "role": e_role, "allowed_pages": e_pages, "machine_perms": e_m_perms, "editable_machine_fields": e_edits})
                        
                    log_security_event(st.session_state["username"], f"SỬA USER ({sel_u})", "Thành công")
                    show_popup_message("THÀNH CÔNG", f"Đã cập nhật thông tin cho {sel_u}!", "💾")

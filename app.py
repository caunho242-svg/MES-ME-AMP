...
# ==========================================
# 🗄️ KẾT NỐI & KHỞI TẠO CƠ SỞ DỮ LIỆU SQLITE
# ==========================================
...
def init_db():
    ...
    # 4. Tạo tài khoản Admin mặc định nếu DB trống
    ...
    # 5. Dữ liệu máy móc mẫu (Nếu trống)
...
# ==========================================
# CÁC HÀM HỖ TRỢ HIỂN THỊ VÀ DỮ LIỆU
# ==========================================
...
if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
    ...
    # ---------------------------------------------------------
    # TRANG 2: QUẢN LÝ MÁY MÓC (TƯƠNG TÁC DB)
    # ---------------------------------------------------------
        ...
        with t_edit:
            if "Chỉnh sửa" in u_perms and machine_db:
                sel_m = st.selectbox("Chọn máy", [f"{m['id']} - {m['name']}" for m in machine_db]).split(" - ")[0]
                ...
                with st.form("edit_m_form"):
                    ...
                    if st.form_submit_button("💾 Cập Nhật", use_container_width=True):
                        conn = get_db_connection()
                        conn.execute("UPDATE machines SET name=?, line=?, url=? WHERE id=?", 
                                     (e_name if "Tên máy" in u_edits else cur_m["name"],
                                      e_line if "Dây chuyền (Line)" in u_edits else cur_m["line"],
                                      e_url if "Đường dẫn máy" in u_edits else cur_m["url"], sel_m))
                        conn.commit()
                        conn.close()
                        show_popup_message("THÀNH CÔNG", f"Đã cập nhật máy {sel_m}!", "💾")
        ...
        with t_del:
            if "Xóa" in u_perms and machine_db:
                ...
                if st.button("🗑️ Xác Nhận Xóa", type="primary", use_container_width=True):
                    ...
                    conn.commit()
                    conn.close()
                    show_popup_message("ĐÃ XÓA", f"Đã xóa máy {del_m}!", "🗑️")

    # ---------------------------------------------------------
    # TRANG 3: QUẢN LÝ TÀI KHOẢN (TƯƠNG TÁC DB)
    # ---------------------------------------------------------
        ...
        t_lst, t_add_u, t_edit_u, t_del_u, t_logs = st.tabs(["📋 Danh Sách", "➕ Tạo Mới", "✏️ Sửa", "🗑️ Xóa", "🛡️ Audit Logs"])

        with t_lst:
            st.dataframe(pd.DataFrame([{"Tài khoản": u["username"], "Họ Tên": u["name"], "Bộ phận": u["department"], "Quyền": u["role"]} for u in users_db]), use_container_width=True)
        ...
        with t_edit_u:
            sel_u = st.selectbox("Chọn tài khoản", [u["username"] for u in users_db])
            ...
            with st.form("edit_user_form"):
                ...
                if st.form_submit_button("💾 Lưu Thay Đổi", use_container_width=True):
                    ...
                    conn = get_db_connection()
                    conn.execute("UPDATE users SET password_hash=?, name=?, allowed_pages=? WHERE username=?", 
                                 (hash_password(e_pass) if e_pass else cur_u["password_hash"], e_name, json.dumps(e_pages), sel_u))
                    conn.commit()
                    conn.close()
                    log_security_event(st.session_state["username"], f"SỬA USER ({sel_u})", "Thành công")
                    show_popup_message("THÀNH CÔNG", f"Đã cập nhật {sel_u}!", "💾")
